import { verdictLabels, reportMessages } from './labels.mjs';
...
export function markdownReport(input, report) {
  ...
  for (const id of ['A','B']) {
    ...
    if (!answer.reasoning.length) lines.push(reportMessages.reasoning,'');
    answer.reasoning.forEach((s,i)=>lines.push(`${i+1}. ${clean(s.explanation)} (${s.basis})`,...(s.quote?['',`> ${clean(s.quote).replace(/\n/g,'\n> ')}`]:[]),''));
    if (answer.strengths.length) lines.push('### Strengths',...answer.strengths.map(s=>`* ${clean(s)}`),'');
    if (answer.weaknesses.length) lines.push('### Weaknesses',...answer.weaknesses.map(s=>`* ${clean(s)}`),'');
  }
  if (report.agreements.length) lines.push('## Agreements',...report.agreements.map(d=>`* ${clean(d)}`),'');
  lines.push('## Differences',...(report.differences.length ? report.differences.map(d=>`* ${clean(d)}`) : [reportMessages.differences]),'','## Claims');
  if (!report.claims.length) lines.push(reportMessages.claims);
